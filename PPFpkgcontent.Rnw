...
\end{center}
 ...
 The main function of the package is called PPforest which implements a projection pursuit random forest. Table \ref{ppforestfun} presents the PPforest function arguments and their respective description.
  \begin{center}
 \begin{table}[!h]
  \begin{tabular}{ l | p{12cm} } \hline
...
The number of trees in the forest is specified using the argument {\tt m}. The argument {\tt size.p} is the sample proportion of the variables used in each node split, PPmethod is the projection pursuit index to be optimized,  two options LDA and PDA are available.
