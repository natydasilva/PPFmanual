...
Function &Description \\ \hline
 ...
 node\_data&Data structure with the  projected and boundary by node and class\\ \\
 ...
 permute\_importance&Obtain the permuted importance variable measure\\ \\
 PPtree\_split&Projection pursuit classification tree with random variable\\
 & selection in each split\\ \\
 ppf\_avg\_pptree\_imp& Computes a global importance measure for a PPforest object, average\\ \\
 ppf\_global\_imp& Computes a global importance measure for a PPforest object\\ \\
 print.PPforest& Print PPforest object\\ \\
 predict.PPforest&Predict class for the test set and calculate prediction error\\
 ternary\_str&Data structure with the  projected and boundary by node and class\\ \\
 tree\_pred&Obtain predicted class for new data using PPforest t.\\ \\
   \hline
  \end{tabular}
...
PPforest package includes some data set that were used to test its predictive performance. The data sets included are summarized in Table \ref{datappf}.

\begin{center}
\begin{table}[!h]
\begin{tabular}{ l | l } \hline
Data & Description\\ \hline
{\tt crab} & Measurements on rock crabs of the genus Leptograpsus \\
{\tt fishcatch} & Measurements on fishes caught form Finland\\
{\tt glass} & Measurements on 6 different types of class\\
{\tt image} & Instances from 7 outoor images\\
{\tt leukemia} & Gene expression data set in two types of acute leukemias\\
{\tt lymphoma} & Gene expression in the three most prevalent adult lymphoid malignancies\\
{\tt NCI60} & Gene expression data among the 60 cell lines\\
{\tt parkinson} & Data set containing 195 observations from 2 parkinson types \\
...
\end{center}
 ...
 The main function of the package is PPforest which implements a projection pursuit random forest. Table \ref{ppforestfun} presents the PPforest function arguments and their respective description.
 
  \begin{center}
 \begin{table}[!h]
  \begin{tabular}{ l | l } \hline
  Arguments & Description\\ \hline
...
logical condition, if it is TRUE then parallelize the function\\ \hline
 {\tt cores} &number of cores used in the parallelization\\ \hline
  \end{tabular}
  \caption{Summary of available arguments and value returned by the function PPforest \label{ppforestfun}}
  \end{table}
  \end{center}
...
%   
% PPtree\_split this function implements a projection pursuit classification tree with random variable selection in each split, based on the original PPtreeViz algorithm. This function returns a PPtreeclass object.
%  To use this function we need to specify a formula describing the model to be fitted response$\sim$predictors(form), data is a data frame with the complete data set. Also we need to specify the method PPmethod, it is the index to use for projection pursuit: ``LDA'' or ``PDA'',
...
% @
% 
% \textbf{PPforest} function runs a projection pursuit random forest.  The arguments are a data frame with the data information, class with the name of the class variable argument.  size.tr to specify the proportion of observations using in the training. Using this function we have the option to split the data in training and test using \textbf{size.tr} directly. size.tr is the proportion of data used in the training and the test proportion will be 1- size.tr.
...
% @
% 
% \textbf{PPforest} print a summary result from the model with the confusion matrix information and the oob-error rate in a similar way randomForest packages does.
% 
