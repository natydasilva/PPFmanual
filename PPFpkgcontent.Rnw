...
This function grow a projection pursuit tree for each bootstrap sample and random sample selection in each node partition.
...
Figure \ref{mos} shows an additionall visualization using the information from {\tt dats\_node} function. This figure is a mosaic plot for the confusion table for each split.
\begin{figure}[!hbpt]
<<echo=TRUE, fig.height=4, fig.width=8, message=FALSE>>=
dat_mosaic <-
...
This function predict the class for the test set and compute the predicted error rate from a PPtree object.
...
Table \ref{ternafun} presents a summary with the available arguments for this function.
 \begin{center}
 \begin{table}[!h]
 \caption{Summary of available arguments of function {\tt ternary} \label{ternafun}}
  \begin{tabular}{ l | p{12cm} } \hline
  \textbf{Arguments} & \textbf{Description}\\ \hline
{\tt ppf}	&Is a PPforest object\\ \hline
...
{\tt dy}	&Second direction included in id\\ \hline
 \end{tabular}
  \end{table}
  \end{center}
This example shows how to use the output from  {\tt ternary\_str} can be used to visualize a genaralized ternary plot using vote matrix information.
\begin{figure}[!hbpt]
 <<echo=TRUE, fig.height=4, fig.width=8, message=FALSE>>=
